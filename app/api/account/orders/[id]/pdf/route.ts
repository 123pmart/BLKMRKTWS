...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const order = await getOrderForVerifiedStore(identity, decodeURIComponent((await params).id));
  if (!order) return Response.json({ ok: false, message: "Order not found" }, { status: 404, headers: { "Cache-Control": "no-store" } });
