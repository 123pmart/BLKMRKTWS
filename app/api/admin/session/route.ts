import { adminAuthConfigured, clearAdminSession, isAdminRequest, setAdminSession, verifyAdminCredentials } from "@/lib/admin/auth";
...
export async function GET(request: Request) {
  return Response.json({ ok: true, authenticated: await isAdminRequest(request), configured: adminAuthConfigured() }, { headers: { "Cache-Control": "private, no-store" } });
}
